...
read_cov <- function(file_, outdir_, nchunks_) {
  ...
  for (i in 1:nchunks_) {
    ...
    mutate(split_[[i]],
           ...
           TEX_f = (((TEXp_A_f + TEXp_B_f + TEXp_C_f) / TEXp_sum) - ((TEXm_A_f + TEXm_B_f + TEXm_C_f) / TEXm_sum)) * 1e9,
           TEX_r = (((TEXp_A_r + TEXp_B_r + TEXp_C_r) / TEXp_sum) - ((TEXm_A_r + TEXm_B_r + TEXm_C_r) / TEXm_sum)) * 1e9) |>
      mutate(TEX_f = ifelse(TEX_f < 0 , 0, TEX_f),
             TEX_r = ifelse(TEX_r < 0 , 0, TEX_r)) |>
      select(contig, position, PRO_f, PRO_r, TEX_f, TEX_r) |>
